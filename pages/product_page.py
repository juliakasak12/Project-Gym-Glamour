...
class ProductPage(BasePage):
    ...
    def _verify_page(self):
        print("Weryfikacja strony produktu")
        go_to_basket_button = self.driver.find_element(*ProductPageLocators.GO_TO_BASKET_BUTTON)
        self.wait_5s.until(EC.element_to_be_clickable(go_to_basket_button))
        add_to_card_button = self.driver.find_element(*ProductPageLocators.ADD_TO_CARD_BUTTON)
        self.wait_5s.until(EC.element_to_be_clickable(add_to_card_button))
        black_color_button = self.driver.find_element(*ProductPageLocators.BLACK_COLOR_BUTTON)
        self.wait_5s.until(EC.element_to_be_clickable(black_color_button))
